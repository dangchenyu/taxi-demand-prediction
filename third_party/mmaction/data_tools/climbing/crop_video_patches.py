...
def crop(img, x_c, y_c, window_size):
    ...
    if x_c < half_window:
        padded_img = cv2.copyMakeBorder(padded_img, 0, 0, half_window, 0, borderType=cv2.BORDER_REFLECT)
        x_base = half_window
    if x_c > img.shape[1] - half_window:
        padded_img = cv2.copyMakeBorder(padded_img, 0, 0, 0, half_window, borderType=cv2.BORDER_REFLECT)
    if y_c < half_window:
        padded_img = cv2.copyMakeBorder(padded_img, half_window, 0, 0, 0, borderType=cv2.BORDER_REFLECT)
        y_base = half_window
    if y_c > img.shape[0] - half_window:
        padded_img = cv2.copyMakeBorder(padded_img, 0, half_window, 0, 0, borderType=cv2.BORDER_REFLECT)
...
if __name__ == '__main__':
    ...
    for line in annotation_file:
        ...
        base_video_name = os.path.split(video_name)[1].split('.')[0]
        ...
        data = np.loadtxt(os.path.join(args.track_dir, action, base_video_name + '.txt'), delimiter=',')
        data = data[:, :6]
        ...
        for detection in data:
            ...
            img = cv2.imread(os.path.join(args.rawframes_dir, action,base_video_name, '{}.png'.format(int(detection[0]))))
            if img is None:
                print('Frame {} Not Found'.format(
                    os.path.join(args.rawframes_dir,action, base_video_name, '{}.png'.format(int(detection[0])))))
                break
