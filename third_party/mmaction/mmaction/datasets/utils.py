...
def parse_other_dataset_splits(level, dataset_path):
    class_ind = [x.strip().split()
                 for x in open(dataset_path + '/annotations/classInd.txt')]
    class_mapping = {x[1]: int(x[0]) - 1 for x in class_ind}
    ...
    for i in range(1, 2):
        train_list = [line2rec(x) for x in open(
            dataset_path + '/annotations/trainlist{:02d}.txt'.format(i))]
        test_list = [line2rec(x) for x in open(
            dataset_path + '/annotations/testlist{:02d}.txt'.format(i))]
        splits.append((train_list, test_list))
    return splits
